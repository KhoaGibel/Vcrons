...
// ==========================================
// 1. DỮ LIỆU & COMPONENT DÙNG CHUNG
// ==========================================
      ...
      <div className="product-card" style={{
        ...
        <div style={{ width: "100%", aspectRatio: "4/3", background: colorMap[product.color] || "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          {product.image ? (
            ...
            <span style={{ fontSize: "13px", color: "rgba(0,0,0,0.35)", letterSpacing: "1px", textTransform: "uppercase" }}>{product.color}</span>
          )}
          <div style={{ position: "absolute", top: "12px", left: "12px", display: "flex", flexDirection: "column", gap: "5px" }}>
            {product.isNew && <span style={{ background: "#1a1a1a", color: "#fff", fontSize: "10px", fontWeight: 700, letterSpacing: "1.5px", padding: "3px 8px", borderRadius: "2px" }}>MỚI</span>}
            {discount && <span style={{ background: "#e60000", color: "#fff", fontSize: "10px", fontWeight: 700, letterSpacing: "1px", padding: "3px 8px", borderRadius: "2px" }}>-{discount}%</span>}
          </div>
        </div>
        <div style={{ padding: "16px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span className={`product-price ${product.isSale ? "sale-price" : ""}`} style={{ fontSize: "16px", fontWeight: 700, color: product.isSale ? "#e60000" : "#1a1a1a" }}>
              {formatPrice(product.price)}
            </span>
            {product.originalPrice && <span style={{ fontSize: "13px", color: "#bbb", textDecoration: "line-through" }}>{formatPrice(product.originalPrice)}</span>}
          </div>
        </div>
      </div>
    </Link>
...
// ==========================================
// 2. GIAO DIỆN TRANG CHỦ CHÍNH
// ==========================================
const Home = () => {
  // 1. Khai báo danh sách ảnh Banner của bạn
const banners = [banner1, banner2];
  ...
  return (
    <div className="home-page" style={{ minHeight: "100vh", backgroundColor: "#fafafa", fontFamily: "'Lato', sans-serif", transition: "background-color 0.3s ease" }}>
      
      {/* --- PHẦN BANNER CHUYỂN ĐỘNG --- */}
      <div className="hero-banner" style={{ width: "100%", height: "75vh", minHeight: "550px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 10, textAlign: "center", color: "#fff", padding: "0 20px", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              ...
              <h1 style={{ fontSize: "clamp(45px, 7vw, 85px)", fontWeight: 900, textTransform: "uppercase", letterSpacing: "3px", margin: "0 0 35px", textShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>
                  VCRONS SUMMER
              </h1>
              <Link to="/shop" style={{ 
                  display: "inline-block", backgroundColor: "#fff", color: "#1a1a1a", 
                  padding: "18px 48px", fontSize: "15px", fontWeight: 900, letterSpacing: "2px", 
                  textTransform: "uppercase", textDecoration: "none", borderRadius: "2px",
                  transition: "all 0.3s ease", boxShadow: "0 4px 20px rgba(0,0,0,0.2)"
              }}>
                  Mua Ngay
              </Link>
              ...
              <div style={{ position: "absolute", bottom: "30px", display: "flex", gap: "12px" }}>
                  {banners.map((_, i) => (
                      <div key={i} onClick={() => setCurrentSlide(i)}
                        style={{ width: "12px", height: "12px", borderRadius: "50%", cursor: "pointer", backgroundColor: currentSlide === i ? "#fff" : "rgba(255,255,255,0.4)", transition: "all 0.3s ease" }} 
                      />
                  ))}
              </div>
          </div>
      ...
      <div className="container" style={{ padding: "80px 15px" }}>
        <div style={{ textAlign: "center", marginBottom: "48px" }}>
          <h2 className="home-section-title" style={{ fontSize: "28px", fontWeight: 900, letterSpacing: "3px", textTransform: "uppercase", color: "#1a1a1a", margin: "0 0 12px" }}>
            Sản Phẩm Nổi Bật
          </h2>
        ...
        <div style={{ textAlign: "center" }}>
          <Link to="/shop" className="view-all-btn" style={{
            display: "inline-block", padding: "12px 32px", border: "2px solid #1a1a1a", color: "#1a1a1a",
            fontSize: "13px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", textDecoration: "none", transition: "all 0.3s ease"
          }}>
            Xem tất cả sản phẩm
          </Link>
        </div>
